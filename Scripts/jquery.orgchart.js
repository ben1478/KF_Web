...
(function ($) {
    ...
    $.fn.orgChart.defaults = {
        ...
        nodeID: function ($node) { return $node.clone().children("ul,li").remove().end()[0].id; },
        interactive: false,
        fade: false,
        speed: "slow",
    ...
    function buildNode($node, $appendTo, level, index, opts) {
        var $table = Object();
        if (level == 0) {
            $table = $("<table cellpadding='0' id='tbCharMain' cellspacing='0' border='0'/>");
        }
        else {
        ...
        var $heading = $("<h2 id='C" + opts.nodeID($node) + "'>").html(opts.nodeText($node));
